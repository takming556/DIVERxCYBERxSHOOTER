...
void HomingShot::draw() {
	...
	if (skin_id == SkinID::ICHIGO_CHAN) {
		...
		int HeartHandles = ImageHandles::HEART_RED;		//ImageHandlesの初期化
		int random_num = DxLib::GetRand(8) + 1;			//ランダム数生成
		
		switch (random_num) {							//switch文で分岐
		case 1:
			HeartHandles = ImageHandles::HEART_RED;		//ImageHandlesを割当
			break;
		case 2:
