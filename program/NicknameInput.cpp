...
void NicknameInput::respond_to_keyinput() {
	...
	if (CONFIRMING_FLAG == false) {
		...
		if (KeyPushFlags::LEFT == false && AppSession::KBD_BUFFER[KEY_INPUT_LEFT] == 1) {
			digitslide_left();
			KeyPushFlags::LEFT = true;
		}
		if (KeyPushFlags::LEFT == true && AppSession::KBD_BUFFER[KEY_INPUT_LEFT] == 0) {
			KeyPushFlags::LEFT = false;
		}

		if (KeyPushFlags::RIGHT == false && AppSession::KBD_BUFFER[KEY_INPUT_RIGHT] == 1) {
			digitslide_right();
			KeyPushFlags::RIGHT = true;
		}
		if (KeyPushFlags::RIGHT == true && AppSession::KBD_BUFFER[KEY_INPUT_RIGHT] == 0) {
			KeyPushFlags::RIGHT = false;
		}

		if (KeyPushFlags::Z == false && AppSession::KBD_BUFFER[KEY_INPUT_Z] == 1) {
		...
		if (KeyPushFlags::ENTER == false && AppSession::KBD_BUFFER[KEY_INPUT_RETURN] == 1) {
			determined_flag = true;
			DxLib::PlaySoundMem(SoundHandles::FORWARD, DX_PLAYTYPE_BACK);
			KeyPushFlags::ENTER = true;
		}
		if (KeyPushFlags::ENTER == true && AppSession::KBD_BUFFER[KEY_INPUT_RETURN] == 0) {
